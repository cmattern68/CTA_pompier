...
namespace cta::engine::button {
    RoundedRectangleShapeButton::RoundedRectangleShapeButton(
        ...
        _size = size;
        _color = color;
        _shape = std::make_unique<cta::engine::shape::RoundedRectangleShape>(
            _pos,
            _size,
            _color,
            5.0
    ...
    bool RoundedRectangleShapeButton::isClicked(std::shared_ptr<cta::engine::Window> &window, std::shared_ptr<cta::engine::Event> &evt) {
        if (evt->isMouseReleasedEvent(cta::engine::LeftClick) && _isClickable)
            if (isHovered(cta::engine::Event::getMousePosition(window->getPosition())))
                return true;            
        return false;
    }

    void RoundedRectangleShapeButton::draw(std::shared_ptr<cta::engine::Window> &window) {
